...
{
	return Vector3f(r * pikmin2_cosf(theta), y, r * pikmin2_sinf(theta));
}

Vector3f RotatePoint(const Vector3f& vec, const f32 theta, const Vector3f& origin)
{
	Vector3f originVec = vec - origin;
	return Vector3f(pikmin2_cosf(theta) * originVec.x - pikmin2_sinf(theta) * originVec.z + origin.x, vec.y,
...
Vector2f RotatePoint(const Vector2f& vec, const Vector2f& origin, const f32 theta)
{
	...
	return Vector2f(pikmin2_cosf(theta) * originVec.x - pikmin2_sinf(theta) * originVec.y + origin.x,
...
inline Vector3f Lerp(const f32 val, const Vector3f& first, const Vector3f& second) { return first * (1.0f - val) + second * val; }
